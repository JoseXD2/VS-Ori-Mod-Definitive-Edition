...
class PauseSubState extends MusicBeatSubstate
{
	...
	var menuItemsOG:Array<String> = ['Resume', 'Restart Song', 'Change Difficulty', 'Exit to freeplay', 'Exit to Options', 'Exit to menu'];
	var menuFuckYou:Array<String> = ['Resume', 'Restart Song'];
	var menuONELIFE:Array<String> = ['Resume', 'Exit to freeplay', 'Exit to Options', 'Exit to menu'];
	...
	public function new(x:Float, y:Float)
	{
		...
		if (CoolUtil.difficultyString() != 'ONELIFE' && CoolUtil.difficultyString() != 'EASIER') // fuck you
		...
		if (!PlayState.isStoryMode)
			menuItemsOG.remove('Exit to freeplay'); // No need this option if you are already in freeplay!

		if (SelectGameState.inpurgatory)
			menuItemsOG.insert(3, 'Exit to Bonus'); // No need this option if you are already in freeplay!

		if (CoolUtil.difficultyString() == 'ONELIFE' || CoolUtil.difficultyString() == 'EASIER')
			menuItems = menuONELIFE;
		else
		menuItems = menuItemsOG;
		switch (PlayState.SONG.song.toLowerCase())
		{
			case 'opposition' | 'cheating' | 'practice':
		...
		levelInfo.text = 'Song Name:\n' + PlayState.SONG.song + '\n\nDifficulty:\n' + CoolUtil.difficultyString() + '\n\nFaints:\n' + PlayState.deathCounter + '\n';
		levelInfo.scrollFactor.set();
		levelInfo.y = 100;
		levelInfo.setFormat(Paths.font("funkin.otf"), 50, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		levelInfo.updateHitbox();
		add(levelInfo);

		practiceText = new FlxText(20, 15 + 101, 0, "PRACTICE MODE", 32);
		practiceText.scrollFactor.set();
		practiceText.setFormat(Paths.font('funkin.otf'), 32);
		...
		chartingText.setFormat(Paths.font('funkin.otf'), 32);
		...
		if (accepted)
		{
			...
			switch (daSelected)
			{
				...
				case "Restart Song":
					...
					PlayState.instance.health = 0; // i am so cruel :(
					close();
					}
					else
					restartSong();
				...
				case 'Toggle Botplay': // Skill Issue Mode
