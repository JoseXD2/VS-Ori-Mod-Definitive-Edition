...
class TitleState extends MusicBeatState
{
	...
	var gradientBar:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width, 1, 0xFF5E93FF);
	...
	public static var completewarnshit:Bool = false;
	...
	//	if(FlxG.save.data.flashing == null && !FlashingState.leftState) {
		if (!ClientPrefs.skipwarnings && !completewarnshit) {
			FlxTransitionableState.skipNextTransIn = true;
			completewarnshit = true;
	...
	/*	if (ClientPrefs.menuMusic == 'Tea Time')
		...
		if (ClientPrefs.menuMusic == 'Dave and Bambi')
		Conductor.changeBPM(160);
		if (ClientPrefs.menuMusic == 'Funky')
		Conductor.changeBPM(70);*/
		else if (ClientPrefs.menuMusic == 'Getting Freaky' || ClientPrefs.menuMusic == 'B-Sides' || ClientPrefs.menuMusic == 'Bambi\'s Purgatory' || ClientPrefs.menuMusic == 'Still')
	...
	//	FlxTween.angle(FlxG.camera, FlxG.camera.angle, 90, 0.01, {ease: FlxEase.quartInOut});
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 24, 0, "Psych Forever Engine (Spirit Engine) v" + MainMenuState.psychForeverVersion + " (PE v" + MainMenuState.psychEngineVersion + " // FNF v0.2.8)", 12);
		...
		ngSpr = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('finnyoung'));
		add(ngSpr);
		ngSpr.visible = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
				//	if (ClientPrefs.menuMusic != 'Still')
				//	FlxG.sound.music.stop();
					#if !html5
					if (ClientPrefs.skipwarnings)
					MusicBeatState.switchState(new SelectGameState());
					else
					MusicBeatState.switchState(new OutdatedState());
					#else
					MusicBeatState.switchState(new AntiPiracyState()); // FUCK YOU!!!!!!!!!!!!!!!
					#end
					closedState = true;
				});
	...
	override function beatHit()
	{
		...
		logoBl.scale.x = 0.775;
		logoBl.scale.y = 0.775;
		logoTween = FlxTween.tween(logoBl.scale, {x: 0.7, y: 0.7}, 0.2, {
		...
		if(!closedState) {
			if (ClientPrefs.menuMusic == 'Bambi\'s Purgatory')
			sickBeats = 16;
			else
			sickBeats++;
			switch (sickBeats)
			{
				case 1:
				//	FlxTween.angle(FlxG.camera, FlxG.camera.angle, 0, 1, {ease: FlxEase.quartInOut});
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			else //Default! Edit this one!!
			{
				...
				gradientBar = FlxGradient.createGradientFlxSprite(Math.round(FlxG.width), 512, [0x00, 0xFFB2C4FF, 0xFF5E93FF], 1, 90, true);
				gradientBar.y = FlxG.height - gradientBar.height;
