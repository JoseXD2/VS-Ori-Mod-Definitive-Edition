...
{
	var options:Array<String> = ['Note Colors', 'Controls', 'Adjust Delay and Combo', 'Preferences', 'Psych Forever Settings', 'Modifiers', 'Modded Specific'];
	...
	//	FlxG.sound.playMusic(Paths.music(Paths.formatToSongPath('configurator')));
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('blankLOL'));
		bg.updateHitbox();
	...
	override function update(elapsed:Float) {
		...
		if (controls.BACK) {
			...
			if (SelectGameState.memorizestate == 'select')
			MusicBeatState.switchState(new SelectGameState());
			if (SelectGameState.memorizestate == 'purgatory')
			MusicBeatState.switchState(new PURMainMenuState());
			if (SelectGameState.memorizestate == 'main')
