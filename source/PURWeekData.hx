...
class PURWeekData {
	...
	public static function createWeekFile():PURWeekFile {
		var weekFile:PURWeekFile = {
	...
	public function new(weekFile:PURWeekFile, fileName:String) {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		var sexList:Array<String> = CoolUtil.coolTextFile(Paths.getPreloadPath('purweek/weekList.txt'));
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				var fileToCheck:String = directories[j] + 'purweek/' + sexList[i] + '.json';
				if(!weeksLoaded.exists(sexList[i])) {
					var week:PURWeekFile = getWeekFile(fileToCheck);
					if(week != null) {
						var weekFile:PURWeekData = new PURWeekData(week, sexList[i]);
		...
		for (i in 0...directories.length) {
			var directory:String = directories[i] + 'purweek/';
	...
	private static function addWeek(weekToCheck:String, path:String, directory:String, i:Int, originalLength:Int)
	{
		...
		{
			var week:PURWeekFile = getWeekFile(path);
			if(week != null)
			{
				var weekFile:PURWeekData = new PURWeekData(week, weekToCheck);
				if(i >= originalLength)
				{
					#if windows
	...
	private static function getWeekFile(path:String):PURWeekFile {
